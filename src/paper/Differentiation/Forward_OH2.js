...
var y, error, exact;
class Forward_OH2 extends Component {
    constructor() {
        super();
        this.state = {
            fx: "",
    ...
    databaseAPI = async()=>{
        var response = await axios.get('http://localhost:3010/api/users/showforward2').then(res => {return res.data});
        this.setState({
        ...
        alert(this.state.x)
        alert(this.state.h)
        this.Forward_OH2(this.state.x,this.state.h,this.state.Order);
    }
    Forward_OH2(x, h, degree) {
        switch (degree) {
            case 1:
    ...
    render() {
        return(
            ...
            <div>
                ...
                <span style={{ color: 'Black', fontSize: '24px' }}>Forward Divided-Difference O(h{<sup>2</sup>})</span>
                <br></br><br></br>
                    <Card
                    ...
                    >
                        {this.state.showinput &&
                            <div>
                                ...
                                <Button type="primary" size="large" onClick= {()=>this.Forward_OH2(parseFloat(this.state.x), parseFloat(this.state.h), parseInt(this.state.degree))}>Submit</Button><br></br><br></br>
